...
def explore_random():
    ...
    room_id = player.current_room["room_id"]
...
def dft_for_dead_end():
    ...
    time.sleep(1)
    while '?' in list(player.graph[current_id].values()):
        current_id = player.current_room["room_id"]
...
def generate_path(target):
    ...
    q = Queue()
    q.enqueue([player.current_room["room_id"]])
...
def travel_to_target(target='?'):
    ...
    while bfs_path is not None and len(bfs_path) > 0:
        ...
        next_direction = next((k for k, v in player.graph[current_id].items() if v == next_room), None)
